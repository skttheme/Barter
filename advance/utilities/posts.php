...
if(!function_exists('barter_posted_by_author')){
	...
	function barter_posted_by_author() {
        ...
        return
			...
			sprintf(
			esc_html__( 'by %s ', 'barter' ),
			sprintf(
				'<a class="vcard author" href="%1$s" title="%2$s" %4$s><span class="fn" %5$s>%3$s</span></a>',
				esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ),
				/* translators: %s - author name */
				esc_attr( sprintf( __( 'View all posts by %s', 'barter' ), get_the_author() ) ),
				esc_html( get_the_author() ),
				wp_kses_post( barter_get_schema_args('author') ),
				wp_kses_post( barter_get_schema_args('name') )
			)
