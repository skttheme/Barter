...
add_action( 'after_switch_theme', 'barter_switch_theme'   );
add_action( 'load-customize.php', 'barter_load_customize' );
add_action( 'template_redirect',  'barter_preview'        );
 ...
 * @return string
 */
function barter_compat_message() {
	if ( version_compare( $GLOBALS['wp_version'], BARTER_MIN_WP_VERSION, '<' ) ) {

		return sprintf(
		/* translators: %1$s: Theme name, %2$s: the required WordPress version, %3$s: user's current version */
			__( '%1$s theme requires at least WordPress version %2$s. You are running version %3$s. Please upgrade and try again.', 'barter' ),
			BARTER_OPTIONS_NAME_PART,
			BARTER_MIN_WP_VERSION,
			$GLOBALS['wp_version']
		);

	} elseif ( version_compare( PHP_VERSION, BARTER_MIN_PHP_VERSION, '<' ) ) {

		return sprintf(
		/* translators: %1$s: Theme name, %2$s: required PHP version, %3$s: current PHP version */
			__( '%1$s theme requires at least PHP version %2$s. You are running version %3$s. Please upgrade and try again.', 'barter' ),
			BARTER_OPTIONS_NAME_PART,
			BARTER_MIN_PHP_VERSION,
			PHP_VERSION
		);
	}
 ...
 * @return void
 */
function barter_switch_theme( $old_name ) {

	switch_theme( $old_name ? $old_name : WP_DEFAULT_THEME );

	unset( $_GET['activated'] );

	add_action( 'admin_notices', 'barter_upgrade_notice' );
}
 ...
 * @return void
 */
function barter_upgrade_notice() {

	printf( '<div class="error"><p>%s</p></div>', esc_html( barter_compat_message() ) );
}
 ...
 * @return void
 */
function barter_load_customize() {

	wp_die( esc_html( barter_compat_message() ), '', array( 'back_link' => true ) );
}
 ...
 * @return void
 */
function barter_preview() {

	if ( isset( $_GET['preview'] ) ) { // WPCS: CSRF ok.
		wp_die( esc_html( barter_compat_message() ) );
	}
}
